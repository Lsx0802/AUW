...
from uncertainty_cul import cal_u,cal_u_resnet
...
class WSDDN_res(nn.Module):
    def __init__(self,  blocks_num =[3, 4, 6, 3], block=BasicBlock,numclasses=2):
        ...
        self.f7 = nn.Linear(1024, 1024)
        self.f8c = nn.Linear(1024, numclasses)
        self.f8d = nn.Linear(1024, numclasses)
    ...
    def forward(self, x,ssw_get):
        ...
        x = F.relu(self.f6(x))
        x = F.relu(self.f7(x))
        x_c = F.relu(self.f8c(x))
        x_d = F.relu(self.f8d(x))
        ...
        x1 = segma_c * segma_d
        x = torch.sum(x1, dim=1)

        return x, segma_c, segma_d,u
