...
def main():
    ...
    image_path = r'C:\Users\hello\PycharmProjects\tongue\data_TM\img'  # tongue data set path
    train_txt_path = 'txt/train3_t.txt'
    val_txt_path = 'txt/val3_t.txt'
    ssw_path = 'txt/ssw.txt'
    save_name = 'u_3_3'
    ...
    if model_use_pretrain_weight:
        model_weight_path = "pre_weight/resnet34_t_pre_2.pkl"
    ...
    for epoch in range(EPOCH):
        ...
        for i, (train_data_batch, train_box_batch, train_label_batch) in tqdm(enumerate(data_loader_train),
            ...
            train_outputs, train_op2, train_op3,train_un = model(train_data_batch, train_box_batch)
            train_u=train_un.detach().cpu().numpy().sum()
            loss = criterion(train_outputs, train_label_batch)+train_u*0.001
            # print(loss)
            # 反向传播优化网络参数
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for i, (val_data_batch, val_box_batch, val_label_batch) in tqdm(enumerate(data_loader_val),
                ...
                val_outputs, val_op2, val_op3,val_un = model(val_data_batch, val_box_batch)
                val_u=val_un.detach().cpu().numpy().sum()
                loss = criterion(val_outputs, val_label_batch)+val_u*0.001
