...
def pre(img, label):
    ...
    # load model weights
    weights_path = "Resnet34_t_WSDDN.pkl"
        ...
        for j in range(val_outputs.size(0)):
            for k in range(val_op2.size(1)):
                # if val_op3[j, k, 1] >(1/val_op3.size(1)) or val_op3[j, k, 0] >(1/val_op3.size(1)):
                    new_line = [float('%.3f' % val_op3[j, k, 1].item()),
                                box1[4*k+0],
...
def plot_box(img, box):
    ...
    # img=cv2.putText(img, str(box[0]), (int(box[1]), int(box[2])), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color=(0, 255,0), thickness=1)
    plt.imshow(img,cmap='gray')
