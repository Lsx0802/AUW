...
def pre(img, label):
    ...
    with torch.no_grad():
        ...
        box = box.to(device)
        val_outputs, val_op2, val_op3,val_un= model(img, box)
        # f = open('box_result2.txt', 'w')
        box_2=[]
        for j in range(val_outputs.size(0)):
            for k in range(val_op2.size(1)):
                # if ((val_op3[j, k, 1] >(1/val_op3.size(1)) or val_op3[j, k, 0] >(1/val_op3.size(1))) and val_un[j,k,0]>(1/val_op3.size(1))):
                if val_un[j,k,0]>val_un.mean():
...
def main(fold):
    ...
    path = r'C:\Users\hello\PycharmProjects\tongue\data_TM\img'
...
if __name__ == '__main__':
    ...
    path=r'C:\Users\hello\PycharmProjects\tongue\data_TM\img\140829091953.png'
    # path = r'C:\Users\hello\PycharmProjects\tongue\data_TM\img\140902091603.png'
    label = 'TM'
    single(path,label)
