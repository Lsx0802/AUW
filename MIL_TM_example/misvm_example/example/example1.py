...
def model_l(path):
    ...
    # load model weights
    weights_path = "weight/Resnet34_i_pre_4_2.pkl"
    ...
    with torch.no_grad():
        ...
        # feature,output = torch.squeeze(model(img.to(device))).cpu()
...
def main():
    ...
    # load image
    img_path1 = "data/instance"
    assert os.path.exists(img_path1), "file: '{}' dose not exist.".format(img_path1)
    img_path2 = os.listdir(img_path1)


    bag_val_feature = []
    bag_train_feature=[]

    for j in tqdm(train0):
        patient_feature = []
        for i in tqdm(img_path2):
            if i[0:12] == j[0:12]:
                img_path3 = os.path.join(img_path1, i)
                feature = model_l(img_path3)
                patient_feature.append(feature)

        bag_train_feature.append(np.array(patient_feature))

    for j in tqdm(train1):
        patient_feature = []
        for i in tqdm(img_path2):
            if i[0:12] == j[0:12]:
                img_path3 = os.path.join(img_path1, i)
    ...
    for j in tqdm(val0):
        ...
        bag_val_feature.append(np.array(patient_feature))
    for j in tqdm(val1):
        patient_feature = []
        for i in tqdm(img_path2):
            if i[0:12] == j[0:12]:
    ...
    classifier= misvm.MISVM(kernel='linear', C=1.0)
