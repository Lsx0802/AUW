...
if __name__ == '__main__':
    ...
    for i in range(len(data)):
        ...
        path_img = os.path.join(path,data[i])

        # path_net = os.path.join(BASE_DIR, "..", "..", "Data", "net_params_72p.pkl")
        path_net=save_name+'.pkl'

        # output_dir = os.path.join(BASE_DIR, "..", "..", "Result", "backward_hook_cam")
        output_dir='save_'+save_name
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        output_dir = os.path.join(output_dir,label[i])
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        ...
        show_cam_on_image(img_show, cam, output_dir,data[i][0:-4])
