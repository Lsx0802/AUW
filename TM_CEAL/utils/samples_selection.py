...
def get_uncertain_samples(pred_prob: np.ndarray, k: int,
    ...
    if criteria == 'cl':
        ...
        uncertain_samples = entropy(pred_prob=pred_prob, k=3*k)
        uncertain_samples = margin_sampling(pred_prob=uncertain_samples, k=2*k)
        uncertain_samples = least_confidence(pred_prob=uncertain_samples, k=k)
    else:
